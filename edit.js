...
document.addEventListener( 'DOMContentLoaded', async () => {
    ...
    await cookieStore.get( COOKIE ).then( ( { value } ) => {
        ...
        Object.keys( data ).filter( e => e !== 'id' ).map( e => {
            ...
            if ( e === 'country' ) loadCountry();

            if ( e === 'state' ) loadState();
...
function loadState() {
    ...
    const value = form.querySelector( '[name="country"]' ).value;
    if ( value === '' ) {
...
form.querySelector( '[name="country"]' ).addEventListener( 'change', loadState );

function loadCity() {

    const country = form.querySelector( '[name="country"]' ).value;
    const value = form.querySelector( '[name="state"]' ).value;
...
form.querySelector( '[name="state"]' ).addEventListener( 'change', loadCity );
